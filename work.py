...
def kill_all():
    subprocess.run(['rosnode', 'kill', '/explore', '/move_base', '/laser_scan_matcher_node'])
#, '/Turning_node', '/Controller_node'])
...
def launch_ros_file():
    global process
    try:
        process = subprocess.call(['roslaunch', 'go1_simulation', 'slam.launch'])
#        process1 = subprocess.call(['rosrun', 'extra', 'controller.py'], stdout=subprocess.PIPE)
#        process2 = subprocess.call(['rosrun', 'extra', 'round.py'], stdout=subprocess.PIPE)      
